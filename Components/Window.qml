...
Item {
    ...
    Item {
        id: outerContent
        anchors.top: privateData.fullContent ? scene.top : statusbar.bottom
        width: scene.width
        height: scene.height - (privateData.fullContent ? 0 : statusbar.height)
        clip:true
        Image {
            id: background
    ...
    StatusBar {
        ...
        active: scene.foreground && !privateData.fullscreen
        opacity: privateData.fullscreen ? 0.0 : 1.0

        states: [
            State {
                name: "fullscreen"
                when: privateData.fullscreen
                PropertyChanges {
                    target: statusbar
                    y: -statusbar.height - privateData.application.toolbarHeight
                }
            }
        ]
        transitions: [
    ...
    states: [
        ...
        State {
            ...
            PropertyChanges {
                target: outerContent
                width: scene.height
                height: scene.width - (privateData.fullContent ? 0 : statusbar.height)
            }
            PropertyChanges {
                target: privateData
                angle: 90
